...
with open('dataset/phishtank.json') as f:
    ...
    for _object in object_list:
        ...
        i+=1
#additional_phishing_list = []
#with open('ALL-phishing-links.txt', 'r') as f:
#    additional_phishing_list = f.readlines()[0:40000]
#phishing_list += additional_phishing_list

data = read_csv('dataset/majestic_million.csv')
legit_list = data['Domain'].tolist()[0:9000]
tlds = []
...
#        write.writerow(data)
#
#with open('dataset/feature_labeled_public_test_hackathon_vcs_2023_feature.csv', 'w') as f:
#    write = csv.writer(f)
#    write.writerow(fields)
#    for data in tqdm(organizer_feature_train, total=len(organizer_feature_train)):
#        write.writerow(data)
with open('dataset/3kphish_9kclean_dataset.csv', 'w') as f:
    write = csv.writer(f)
    write.writerow(fields)
